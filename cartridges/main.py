...
class Defectiveadd(add_defective.Ui_Form):
    ...
    def add_b_defective(self):
        if self.lineEdit_name_pr.text() and self.lineEdit_break.text() and self.lineEdit_flaw.text():
            query_dt = QSqlQuery()
            query_dt.exec(f"INSERT INTO Defective(name_printer, breaking, flaw) VALUES ('{self.lineEdit_name_pr.text()}', '{self.lineEdit_break.text()}', '{self.lineEdit_flaw.text()}')")
            self.update_def()

class Workingadd(add_Working.Ui_Form):
    def __init__(self,def_id,update_work= None):
        super().__init__()
        self.setupUi(self)
...
class Refilledadd(add_Refilled.Ui_Form):
    def __init__(self,ward_id,update_ref):
        super().__init__()
        self.setupUi(self)
    ...
    def add_refilled(self):
            if self.lineEdit_name_printer2.text() and self.lineEdit_diagnostics.text() and self.lineEdit_clearing.text() and self.lineEdit_testing_device.text() and self.lineEdit_necessary():
                query_rt = QSqlQuery()
